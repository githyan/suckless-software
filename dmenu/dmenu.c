...
calcoffsets(void)
{
	...
	if (lines > 0)
		...
		n = mw - (promptw + inputw + TEXTW(numbers) + TEXTW(symbol_1) + TEXTW(symbol_2));
...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		x += w;
		for (item = curr; item != next; item = item->right)
      // x = drawitem(item, x, 0, MIN(TEXT(item->text), mw - x - TEXTW(symbol_2)));
			x = drawitem(item, x, 0, textw_clamp(item->text, mw - x - TEXTW(symbol_2) - TEXTW(numbers)));
		if (next) {
			w = TEXTW(symbol_2);
			drw_setscheme(drw, scheme[SchemeNorm]);

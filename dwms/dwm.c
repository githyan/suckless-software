...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					buf[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColFg], buf, 1);
					i += 7;
				} else if (text[i] == 'b') {
					char buf[8];
					memcpy(buf, (char*)text+i+1, 7);
					buf[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColBg], buf, 1);
					i += 7;
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		// 	continue;
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagSel : SchemeTagsNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
	  if (occ & 1 << i)
      drw_rect(drw, x + boxs, boxs, boxw, boxw, m == selmon && selmon->sel && selmon->sel->tags & 1 << 1, urg & 1 << i);
    x += w;
	}
	w = TEXTW(m->ltsymbol);
...
{
/* /
 * Polybar tray does not raise maprequest event. It must bemanually scanned
 * for. Scanning it too early while the tray is being populated would give wrong dimensions.
 * / */
  if (!selmon->traywin)
    scantray();

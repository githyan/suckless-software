...
enum { SchemeNorm, SchemeSel, SchemeHov, SchemeHid, SchemeStatus, SchemeTagSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm}; /* color schemes */
	...
	if ((w = m->ww - tw - x) > bh) {
      if (n > 0) {
        ...
        for (c = m->clients; c; c = c->next) {
          ...
          drw_text(drw, x, 0, tabw, bh, lrpad / 2, m->sel->name, 0);
          x += tabw;
        }
      } else {
			  drw_setscheme(drw, scheme[SchemeStatus]);
